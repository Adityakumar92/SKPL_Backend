...
function ShowAllUser() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const res = await axios.get("/api/normaluser");
      setUsers(res.data || []);
    } catch (err) {
  ...
  const handleView = (user) => {
    navigate("/users/create", { state: { userToView: user } });
  };

  const handleEdit = (user) => {
    navigate("/users/create", { state: { userToEdit: user } });
  };

  const handleDelete = async (userId) => {
    if (!window.confirm("Are you sure you want to delete this user?")) return;
    try {
      await axios.delete(`/api/normaluser/${userId}`);
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        {roleAndPermission["userManagement"] === "all" && (
          <button
            ...
            onClick={() => navigate("/users/createbackend")}
          >
            Create User
          </button>
        )}
